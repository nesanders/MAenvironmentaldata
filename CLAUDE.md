...
- **CTHRU dataset IDs can be silently retired**: The Comptroller's Socrata datasets (`cthru.data.socrata.com`) are identified by opaque 4x4 slugs (e.g. `rr3a-7twk`) that are not stable long-term — the payroll dataset used by `get_DEP_staff_SODA.py` was retired ~Sept 2026 and replaced by a same-schema successor (`9ttk-7vz6`, "Commonwealth Of Massachusetts Payroll v4"), breaking the weekly workflow with 404s for 4 consecutive runs before being caught. If a CTHRU-based fetch script starts 404ing, search the catalog for a replacement: `curl -s "https://cthru.data.socrata.com/api/catalog/v1?domains=cthru.data.socrata.com&q=<keyword>"` and verify column names match via `curl -s "https://cthru.data.socrata.com/api/views/<slug>.json"` before swapping in the new slug.
