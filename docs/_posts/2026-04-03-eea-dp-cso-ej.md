...
On days following dry conditions (less than 0.05 inches in the prior 48 hours), there is roughly a {{ site.data.facts_MAEEADP_through_2025.rain_freq_dry_pct }}% chance of having any discharge statewide — though, as discussed below, some of these may reflect localized storms missed by the statewide station average. The rate of discharge grows steeply and monotonically with rainfall: across all days with at least 0.05 inches in the prior 48 hours, the chance of a discharge somewhere in the state is ~{{ site.data.facts_MAEEADP_through_2025.rain_freq_wet_pct }}%, rising to about {{ site.data.facts_MAEEADP_through_2025.rain_freq_heavy_pct }}% on heavy-rainfall days (0.75–1.5 inches) and essentially {{ site.data.facts_MAEEADP_through_2025.rain_freq_very_heavy_pct }}% on very heavy days (>1.5 inches).

The 48-hour accumulation window was chosen because CSO systems respond to conditions created by cumulative rainfall — soil saturation, groundwater recharge, and pipe storage filling. Bizer & Kirchhoff ([2022, *Water Science & Technology*](https://iwaponline.com/wst/article/86/11/2848/91816/Regression-modeling-of-combined-sewer-overflows-to)) found that CSOs in their Maryland case study were optimally paired with rainfall events identified over a 48-hour lookback window. Because most cooperative weather stations observe in the morning and attribute the preceding ~24 hours of rain to the observation day, each daily rainfall total — recorded on the morning-observation day — is re-assigned to the previous calendar day, the day most of that rain physically fell, before computing the lookback window. This aligns each discharge event with the physical days of rainfall that preceded it.
...
The y-axis is on a logarithmic scale to spread the large dynamic range of discharge volumes. Discharge volumes tend to increase with rainfall severity (Spearman ρ ≈ {{ site.data.facts_MAEEADP_through_2025.rain_volume_spearman_rho }} across the (day, discharge type) pairs shown), though with a large dynamic range at every rainfall level. While SSO events tend to have limited total volume (often less than a million gallons), they are more associated with very heavy rainfall days, indicating that extreme [stormwater inflow](https://en.wikipedia.org/wiki/Infiltration_and_inflow) is a primary cause of Massachusetts SSOs. 

[Bizer & Kirchhoff](https://iwaponline.com/wst/article/86/11/2848/91816/Regression-modeling-of-combined-sewer-overflows-to) studied CSO systems in Maryland and found that, in that system, discharge frequency increases with total rainfall, such that CSOs had maximal probability to occur above 1.5 inches of rainfall — a pattern consistent with the Massachusetts data shown above.

**What about the dry-weather discharges?** A small number of events occur with near-zero prior-48-hour precipitation — {{ site.data.facts_MAEEADP_through_2025.dry_reports }} reports across {{ site.data.facts_MAEEADP_through_2025.dry_days }} days over the full three-and-a-half-year period, {{ site.data.facts_MAEEADP_through_2025.dry_untreated_reports }} of them untreated CSO discharges. Under [EPA's Nine Minimum Controls](https://www.epa.gov/sites/default/files/2015-10/documents/owm0030_2.pdf) and Massachusetts NPDES permits, [CSO discharges during dry weather are prohibited](https://www.mass.gov/guides/sanitary-sewer-systems-combined-sewer-overflows) and require immediate investigation. When they occur, typical causes include groundwater infiltration into aging pipes, illicit storm drain connections, and pump or equipment failures — all NPDES permit violations that utilities are required to investigate and correct. A definite limitation of this analysis is that it uses only a single statewide average rainfall: a localized storm could trigger a discharge in one municipality while the statewide rainfall average remains near zero. Most apparent dry-weather discharge days involve just one municipality reporting (median of {{ site.data.facts_MAEEADP_through_2025.dry_median_munis_per_day }}), so a localized missed-rainfall explanation cannot be excluded for many of these events; the residual set of genuinely dry-weather discharges is correspondingly small, but each one represents a potential permit violation warranting scrutiny. 
...
With nearly three and a half years of data from Massachusetts's new statewide sewage notification system, several patterns have become clear.

**Environmental justice disparities in sewage burden are large and persistent.** Our regression models estimate a burden ratio: the multiplicative increase in discharge burden when a community's EJ indicator (minority population share, low-income share, or linguistic isolation) doubles. Pooling all 2022–2025 data, the overall estimates are striking: **minorities-majority watersheds receive {{ site.data.facts_MAEEADP_through_2025.depend_cso_MINORPCT_Watershed }} the discharge burden**, low-income-majority watersheds experience {{ site.data.facts_MAEEADP_through_2025.depend_cso_LOWINCPCT_Watershed }}, and linguistically isolated-majority watersheds experience {{ site.data.facts_MAEEADP_through_2025.depend_cso_LINGISOPCT_Watershed }}. These disparities are consistent year-to-year (2011–2025) and are not trending downward.

**Rainfall strongly predicts discharge occurrence, and the small residue of dry-weather discharges warrants investigation.** Once daily precipitation records are aligned to the physical day of rainfall, the chance of a discharge somewhere in the state rises monotonically from roughly {{ site.data.facts_MAEEADP_through_2025.rain_freq_dry_pct }}% on dry days to essentially {{ site.data.facts_MAEEADP_through_2025.rain_freq_very_heavy_pct }}% on very heavy rainfall days (>1.5 inches over the prior 48 hours), and discharge volumes correlate with rainfall severity. This is consistent with stormwater inflow as the dominant driver of Massachusetts's CSO burden, matching behavior reported in other CSO systems. A small residual of apparent dry-weather events remains ({{ site.data.facts_MAEEADP_through_2025.dry_reports }} reports across {{ site.data.facts_MAEEADP_through_2025.dry_days }} days over three and a half years) — though most of these days involve just a single reporting municipality, so some may reflect localized rainfall missed by the statewide station average rather than genuine dry-weather discharges. Those that are genuine violate EPA Nine Minimum Controls and Massachusetts NPDES permits and imply chronic infrastructure problems (groundwater infiltration, illicit connections, pump failures) independent of storms. Systematically investigating these events would be valuable for enforcement and infrastructure planning.

**Significant data gaps remain.** Roughly half of untreated CSO and SSO discharge reports continue to use modeled (rather than metered) volume estimates, limiting the precision of this analysis. Investments in outfall metering would substantially improve the quality and utility of this public dataset.
