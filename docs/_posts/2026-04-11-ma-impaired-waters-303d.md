...
The result is unambiguous: **every single one of the {{ site.data.facts_EPA303d.n_cso_mapped }} matched CSO-reporting waterways is rated "Not Supporting" in the most recent 303(d) cycle.** Not one discharges into a waterbody that fully meets water quality standards. Across all years of CSO data, {{ site.data.facts_EPA303d.vol_not_supporting_bgal }} billion gallons of discharge flowed into waterbodies already confirmed to be failing. Of the {{ site.data.facts_EPA303d.vol_total_bgal }} billion gallons of total reported CSO discharge, {{ site.data.facts_EPA303d.pct_vol_impaired_of_total }}% goes to these confirmed-impaired waterways; the remaining {{ site.data.facts_EPA303d.vol_not_matched_bgal }} billion gallons discharged to waterways we could not match to a 303(d) record.

This is not a coincidence of the data. "Fully Supporting" waters do exist in Massachusetts — the 303(d) dataset includes hundreds of assessment units that meet all their designated uses. But none of them have CSO outfalls. CSO infrastructure is concentrated in older urban areas, and those same urban waterways have carried combined sewage for over a century. The waters are impaired *because of* the same infrastructure that produces the CSOs.

This finding does not prove that CSOs *caused* the specific 303(d) listings — impairment assessments reflect many pollution sources. But it does establish that Massachusetts is not discharging sewage into clean waters and getting away with it. It is discharging into waterbodies that are already failing every standard that defines what "clean water" means under federal law.
