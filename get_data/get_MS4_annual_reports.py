...
FAILURES_YML = "../docs/data/MS4_failures.yml"


def _write_failures_yml():
    """Write a YAML manifest of low-confidence records for display in the data page."""
    if not os.path.exists(EXTRACTED_CSV):
        return
    df = pd.read_csv(EXTRACTED_CSV)
    low = df[df["extraction_confidence"] == "low"].copy()
    with open(FAILURES_YML, "w") as f:
        f.write(f"# Auto-generated by get_MS4_annual_reports.py — do not edit\n")
        f.write(f"count: {len(low)}\n")
        f.write("failures:\n")
        for _, row in low.iterrows():
            filename = str(row.get("source_url", "")).split("/")[-1]
            muni = row.get("municipality") or filename
            year = row.get("report_year")
            notes = str(row.get("extraction_notes", "")).replace('"', "'")[:200]
            f.write(f'  - filename: "{filename}"\n')
            f.write(f'    municipality: "{muni}"\n')
            f.write(f'    report_year: {year}\n')
...
def extract_all(local_paths, index_df, dry_run=False, test_mode=False, yes=False, sample=None):
    ...
    for lp, source_url, pages, cost, is_xfa in pdf_queue:
        ...
        try:
            ...
            # Transient API errors: log but don't write to CSV so next run retries
            if any(s in err for s in ("429", "RESOURCE_EXHAUSTED", "Server disconnected",
                                      "503", "502", "Connection")):
                log.info("  Transient error — skipping CSV write, will retry next run.")
                continue
            result = {
